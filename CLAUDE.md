...
├── population/commuting.qmd ← **Commuting** (People-&-Society dropdown, added 2026-06-19): 2021-Census journey-to-work — mode split by city (100%-stacked bar; car ~84% nationally, transit ~15% in Toronto/Montréal/Vancouver) + average commute duration by city (Toronto longest ~30 min). Data from `build_commute.py` (one-time)
...
│   ├── build_commute.py ← ONE-TIME builder (not weekly): 2021-Census commuting (98-10-0457-01, ~500MB cube chunk-read) → commute mode split + average duration by city → data/population/commute_{mode,duration}.csv. Re-run on the 2026 Census
