...
- **Masters**: all text-bearing SVGs re-set in Radio-Canada and re-exported via
  ...
  bold-resolution verified empirically). **Outlined distribution copies exist**
  (`*-outlined.svg`, generated by build_brand.py via fontTools glyph layout;
  pixel-verified against live-text renders incl. the WebKit trailing-
  letter-space anchor quirk) — share those outside the repo; the live-text
  masters stay the editable sources. PNG exports ship at 2× nominal size.
