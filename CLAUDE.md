...
navbar search tool vs the container content edge]). Tab/SEO title + landing H1 + footer + chart attribution = Canada
...
├── visual_assets/         ← **brand identity (built 2026-06-10/11; governance = brand/DESIGN-CRITERIA.md)**: **the BOTANICAL sugar-maple leaf is the identity at every size via two optical cuts** (`brand/botanical/` — display cut ≥~48px, small cut <~48px [teeth removed, thick stem — survives 16px]; classic leaf RETIRED to `brand/classic/`, flag-true leaf REJECTED [reads RCAF roundel/"Avro Arrow"] → archived). **Default expressive mark = `leaf-botanical-cells.svg`** (six palette-colour cells, straight rays from C=(50,44) threading the lobes, maroon STEM cell; strata = the variety fill, owner: its horizontal "stability" register has a place); `solid/` = 8 palette colours × both cuts; ring `monogram.svg` (+reversed/cells) + lockups on the small cut; north-star secondary motif; `palette.md` = locked 8-colour + 2-tint palette (+ `palette-sheet.svg/.png` one-page infographic). `favicon/` + `social/` (og-card 1200×630 = off-white + CELLS [wired as the site og:image] + og-card-strata variant; github avatar/preview; x-header 1500×500 = X/Bluesky/Mastodon; linkedin-banner 1584×396 + -strata) + `motifs/` (area banner waves, ranking-strip divider) — published at /visual_assets/brand/ via project.resources; `studies/` = active explorations; `initial_drafts/` = archive, never published (zip gitignored); README.md = system rules + the qlmanage→sips export recipe (**qlmanage thumbnails SVGs as letterboxed SQUARES — render square then `sips -c` centre-crop for non-square cards; adding width/height attrs makes it cover-scale instead, worse**). GitHub org avatar + repo social preview must be uploaded MANUALLY in github.com settings (no API for either)
