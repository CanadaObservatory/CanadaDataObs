...
├── innovation/index.qmd   ← **Innovation** (Education-Science-&-Innovation dropdown; §552 v2 BUILT 2026-06-23): the innovation-pipeline gap — business R&D (BERD %GDP), **venture capital %GDP** (OECD `DSD_VC`, 1-row), **triadic patents per million** (quality-controlled invention output, bespoke MSTI `P_TRIAD`/pop), **high-tech exports %** (WB `TX.VAL.TECH.MF.ZS`, 1-row), **government support for business R&D direct-vs-tax** (OECD `DSD_RDTAX`, bespoke; Canada outlined, SR&ED tax-heavy). Descriptive (no scorecard). `fetch_innovation.py` (patents+rd-support bespoke). Researched-not-built extensions (2026-06-23): industrial structure / resources+finance reliance (StatCan 36-10-0434 / OECD `DSD_NAMAIN10@DF_TABLE6`), Canadian inventions owned abroad/by US (OECD `DSD_PATENTS@DF_PATENTS_FOREIGN`), foreign-controlled business R&D (StatCan 27-10-0333), high-growth firms (OECD `DSD_SDBSBD_ISIC4@DF_BD_EMP_HG`); **brain-drain emigration BLOCKED** (no refreshable skilled-emigration series — StatCan emigration ends 2004, OECD DIOC frozen 2015/16)
...
## Data sources (111 indicators / 12 sections)
